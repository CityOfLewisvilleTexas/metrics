            ...
            var App = (function(){
                return {
                    ...
                    getData: function(){
                        ...
                        $.ajax({
                            type: "POST",
                            url: "http://eservices.cityoflewisville.com/citydata/?datasetid=DashResponseTime2&date1="+_date1+"&date2="+_date2+"&datasetformat=jsonp&callback=?",
                            ...
                            success: function(e) {
                                ...
                                //Average Response Time Banner (priority 1 calls)
                                    var _rtArray = App.data1.filter(function(a){ return a.Priority == 1 && a.Category.toUpperCase() != 'FIRE/EMS ASSIST';}).map(function(a){return [a.ResponseTime];}).reduce(function(a,b){return a.concat(b)});
                                    var _rtSum = _rtArray.reduce(function(a,b){return a + b;});
                                    var _rtAvg = parseFloat(_rtSum / _rtArray.length).toFixed(2);
                                    
                                    var _totalCallsArray = App.data1.filter(function(a){ return a.Priority == 1 && a.Category.toUpperCase() != 'FIRE/EMS ASSIST';}).map(function(a){return [a.Total];}).reduce(function(a,b){return a.concat(b)}).reduce(function(a,b){return a + b;});
                                ...
                                //Pie Chart Data (Group By [Category], SUM([Total]))
                                    App.pieData = e.results
                                        .map(function(a){return {Category: a.Category, Total:a.Total};})
                                ...
                                //Column Chart
                                    ...
                                    App.drawChart_Column_ComplaintsForCategory_Historical(e.results,"Traffic",App.colors[0]);
                            }
                        });
                        
                        $.ajax({
                            type: "POST",
                            url: "http://eservices.cityoflewisville.com/citydata/?datasetid=DashResponseTimeHist1&datasetformat=jsonp&callback=?",
                            contentType: "application/json; charset=utf-8",
                            dataType: 'jsonp',
                            success: function(e) {
                                var _data = e.results.map(function(a){return [a.Month,a.Priority1];});
                    ...
                    getCallsPerComplaint: function(complaint){
                        ...
                        $.ajax({
                            type: "POST",
                            url: "http://eservices.cityoflewisville.com/citydata/?datasetid=DashPDCallsPerComplaint&complainttype="+_complaint+"&date1="+_date1+"&date2="+_date2+"&priority="+_priority+"&datasetformat=jsonp&callback=?",
                            contentType: "application/json; charset=utf-8",
                            dataType: 'jsonp',
                            success: function(e) {
                                var _data = e.results.map(function(a){return [a.MasterCallNumber,a.Location,a.Priority,a.FirstPD,a.FirstPDTime,a.FirstFD,a.FirstFDTime,a.Date]});
                    ...
                    drawChart_Column_ComplaintsForCategory_Historical: function(_data,_category, _color) {
                        ...
                        //Filter the data to just the category that was clicked
                            var filteredData = _data
                              .filter(function(a){return a.Category == _category;})
                              .map(function(a,b){ return [a.Complaint,a.ResponseTime,a.ResponseTime,a.Tooltip];});
